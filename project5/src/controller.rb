...
AVATAR_DIR = "./static/avatars".freeze
SESSION_BITS = 32
DATE_FMT = "%d %h %G at %R".freeze
...
module Access
  ...
  def authenticate(user, passwd)
    s_user = sanitize user
    s_pass = sanitize passwd
    db_pass = @db.execute(%{
      SELECT password
        FROM users
        WHERE user = ? 
    ...
    if (db_pass[0] == s_pass && get_user(s_user)) then
      assign_session s_user
    else
      nil
    end
  end
...
module User
  ...
  # users matching the query (by name or description).
    ...
    user_query = sanitize user_query
		...
		#puts
		#puts "CODE TEST: #{query}"
		#puts
   	@db.execute(query, [user_query]).each do |user|
   		users << {
   		:name => user[0],
   		:avatar => user[1],
      	:description => user[2],
  ...
  def register(user, filename, file, password, confirm)
    return false if password != confirm
	  s_user = sanitize user
	  s_filename = sanitize filename
	  s_password = sanitize password

    upload_avatar(filename, file)
    @db.execute(%{
		  INSERT INTO Users(User, Password, Avatar)
		  VALUES (?, ?, ?)
      }, [s_user, s_password, s_filename])
  ...
  def update_prefs(user, session, description, token)
    s_user = sanitize user
    s_desc = sanitize description

    if (!authorize(s_user, session) || @tokens[s_user] != token) then
      return false 
    end
...
module Posts
  ...
  def publish_post(user, session, content, token)
    ...
    @db.execute(query, [s_user, s_content, timestamp.to_s])
    true
  end
